...
function validateForme(){
    ...
    const twitchOK = validateRequired(inputUrl);
    const mailOK = validateMail(inputMail);
    const passwordOK = validatePassword(inputPassword);
    const passwordConfirmOK = validateConfirmationPassword(inputPassword, inputValidationPassword);

    if(usernameOK && twitchOK && mailOK && passwordOK && passwordConfirmOK){
...
function validateUrl(input){
    // Définition du Regex
        const urlRegex = /^https?://(?:www.)?[-a-zA-Z0-9@:%.+~#=]{1,256}.[a-zA-Z0-9()]{1,6}\b(?:[-a-zA-Z0-9()@:%+.~#?&/=]*)$/;
        const urlUser = inputUrl.value;
        if(urlUser.match(urlRegex)){
            input.classList.add("is-valid");
            input.classList.remove("is-invalid");
            return true;
        }
